...
{scrollingClients.map((client, index) => (
  <div key={index} className="group relative mx-4" // Cards lambe hain toh margin thoda kam kiya
    style={{
      perspective: "1200px",
    }} >
    <a href={client.url}
      ...
      className="
        relative flex h-36 w-36 items-center justify-center overflow-hidden rounded-2xl border
        border-white/10 bg-white/5 backdrop-blur-xl transition-all
        duration-700 hover:-translate-y-3 hover:scale-105 
      " // Width kam (w-36) aur Height zyada (h-44) kar di hai
      style={{ 
        transformStyle: "preserve-3d", boxShadow: "0 20px 40px rgba(0,0,0,0.45), inset 0 1px 1px rgba(255,255,255,0.15)",
      }} >
      ...
      <div
        ...
        style={{
          boxShadow: "0 0 30px rgba(59,130,246,0.5), 0 0 60px rgba(6,182,212,0.25)",
        }} />

      {/* Top Highlight */}
      <div className="absolute top-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-white/70 to-transparent" />

      {/* Logo - Full view without cropping */}
      <img src={client.logo} alt={client.name} 
        className="relative z-10 h-full w-full object-cover
        grayscale transition-all duration-700 group-hover:grayscale-0 group-hover:scale-110"
        /* object-contain + p-3 se image ka koi bhi kona nahi katega aur full logo show hoga */
        style={{
          transform: "translateZ(50px)", filter: "drop-shadow(0px 8px 16px rgba(0,0,0,0.35))",
        }} />

      {/* Bottom Glow */}
      <div className="absolute bottom-0 left-1/2 h-6 w-20 -translate-x-1/2 
          rounded-full bg-blue-500/30 blur-xl opacity-0 group-hover:opacity-100 transition-all duration-700" 
      />
    </a>

    {/* Company Name */}
    <div className="absolute -bottom-10 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-all duration-500">
      <div className="rounded-full border border-blue-400/30 bg-slate-900/95 px-3 py-1.5 backdrop-blur-xl shadow-2xl">
        <span className="text-[10px] font-semibold text-blue-200 whitespace-nowrap">
          {client.name}
        </span>
      </div>
    </div>
